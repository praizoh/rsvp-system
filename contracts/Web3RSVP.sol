...
contract Web3RSVP {
    ...
    function createNewRSVP(bytes32 eventId) external payable{
        ...
        require(myEvent.confirmedRSVPs.length<myEvent.maxCapacity, "This event has reached max capapcity");
    ...
    function confirmAttendee(bytes32 eventId, address attendee) public{
        ...
        if(myEvent.claimedRSVPs.length==myEvent.confirmedRSVPs.length){
    ...
    function withdrawUnclaimedDeposits(bytes32 eventId) external{
        ...
        require(!myEvent.paidOut, "PAID OUT ALREADY");
        // check if it's been 7 days past myEvent.eventTimestamp
        require(
            block.timestamp >= (myEvent.eventTimestamp + 7 days),
            "TOO EARLY"
        );
        require(msg.sender==myEvent.eventOwner, "NOT EVENT OWNER, THIEF!!!");
